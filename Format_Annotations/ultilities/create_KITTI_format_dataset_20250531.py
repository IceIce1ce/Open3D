...
train_val_ratio         = 10 # 9 images for training, 1 image for validation
train_ratio             = 5
test_ratio              = 3
...
def create_info(scene_name, split: Optional[str] = "Train"):
	info = {
...
def create_images_json(scene_name):
	...
	folder_output_json = "/media/sugarubuntu/DataSKKU3/3_Dataset/AI_City_Challenge/2025/Track_1/MTMC_Tracking_2025/ExtractFrames/KITTI_annotations_format/"
	...
	for camera_path in tqdm(list_camera, desc=f"Processing {scene_name}"):
		...
		for img_path in tqdm(list_img, desc=f"Processing {camera_name}"):
			...
			img_path_short = img_path.replace(folder_input_frame, "")
...
def create_annotations_json(scene_name):
	...
	for img_index in tqdm(range(number_image_per_camera), desc=f"Processing {scene_name}"):
		...
		for instance_key in map_world.instances:
			...
			for camera in map_world.cameras:
				...
				bbox3D_cam = instance.get_3d_bounding_box_on_2d_image_coordinate(
				...
				annotation = {
					...
					"dimensions"	: instance.frames[str(img_index)]["scale_3d"],	# 3D attributes for object dimensions in meters
					"R_cam"			: instance.frames[str(img_index)]["rotation_3d"],				# 3D rotation matrix to the camera frame rotation
				...
				ratio = get_image_id(camera_index, img_index) % (train_ratio + test_ratio)
				if ratio < train_ratio:
...
def combine_all_components(scene_name):
	...
	data_json = {
		"info"       : create_info(scene_name, split="Test"),
		"categories ": categories,
