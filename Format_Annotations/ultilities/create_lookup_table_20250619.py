...
def add_object_to_trajectory_middle_result_postprocess(scene_name, json_data):
...
def main_rotated_3d_bbox_object_txt():
	# initialize paths
	folder_data_version  = "MTMC_Tracking_2025_20250614"
	final_result_path_in = f"/media/sugarubuntu/DataSKKU3/3_Dataset/AI_City_Challenge/2025/Track_1/{folder_data_version}/ExtractFrames/lookup_table/final_result_w-max-l-max-h-avg_linear.txt"
...
def main_add_object_to_trajectory_middle_result_postprocess():
	folder_data_version = "MTMC_Tracking_2025_20250614"
	...
	for scene_name in tqdm(list_scene):
		if scene_name not in ["Warehouse_019", "Warehouse_020"]:
			continue
		...
		json_data = add_object_to_trajectory_middle_result_postprocess(scene_name, json_data)
...
def create_trajectory_middle_result_postprocess(scene_name, json_path_lookup_table, data_preprocess, folder_output, postfix ="avg"):
	...
	for object_id in tqdm(json_data_output, desc=f"Processing creating postprocess in {scene_name}"):
		...
		fps  = 0.0
		yaw_kf      = YawKalmanFilter()
		# location_kf = KalmanFilter2D()
		...
		for img_index in range(number_image_per_camera):
			...
			if frame_current_name != frame_check_name:
				...
				queue_xs.append(measured_x)
				queue_ys.append(measured_y)
				if len(queue_xs) > frame_period * 2:
					queue_xs.pop(0)
					queue_ys.pop(0)
...
def main():
	...
	main_add_object_to_trajectory_middle_result_postprocess()
